...
// Payload INIT_GEOM desde el server Java:
// player(8) + plat[] + vines[] + enemies[] + fruits[] + water[]
// Solo guardamos plat, vines y water (lo demás lo descartamos).
bool cp_recv_init_static_payload(const uint8_t *p, uint32_t len){
    cp_free_static();

    CP_Rect tmp;
    if (!read_rect(&p,&len,&tmp)) return false; // player (descartado)
