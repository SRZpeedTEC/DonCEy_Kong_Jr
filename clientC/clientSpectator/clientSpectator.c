...
static void on_spectator_state(const uint8_t* payloadPtr, uint32_t payloadLen)
{
    ...
    if (payloadLen > 9) {
        ...
        for (uint8_t i = 0; i < entityCount; ++i) {
            ...
            if (kind == 1) {
                uint8_t variant = 0;
                if (spriteId == 1) variant = 0;  // CROC_VARIANT_BLUE
                else if (spriteId == 2) variant = 1;  // CROC_VARIANT_RED
                game_spawn_croc(variant, ex, ey);
            }
            // Spawn fruit (kind 2)
            else if (kind == 2) {
                uint8_t variant = 0;
                if (spriteId == 1) variant = 0;  // FRUIT_VARIANT_APPLE
                else if (spriteId == 2) variant = 1;  // FRUIT_VARIANT_ORANGE
                else if (spriteId == 3) variant = 2;  // FRUIT_VARIANT_BANANA
